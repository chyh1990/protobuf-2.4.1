...
make distclean
export CXXFLAGS="-fPIC -fvisibility=hidden -arch armv7 -arch armv7s -isysroot $IPHONEOS_SYSROOT"
./configure --build=armv7-apple-darwin13.0.0  --disable-shared \
	#--prefix=`pwd`/build-OS --with-protoc=protoc
make -j3 && make install

make distclean
export CXXFLAGS="-fPIC -fvisibility=hidden -arch i386 -isysroot $IPHONESIMULATOR_SYSROOT"
